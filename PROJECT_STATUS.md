# Project Status

This is the live dashboard for Eyuna. It reflects what exists today, what is actively being worked on, and what is planned next. For the full multi-release plan, see [ROADMAP.md](ROADMAP.md).

**Last updated:** 2026-08-01

## Current Version

**v1.0 — Foundation** (in progress)

## Completed

None yet. The repository has just been initialized.

## In Progress

- Repository Initialization — structure, root documentation, directory-level READMEs, and `docs/` placeholders.

## Upcoming

- Constitution (`docs/constitution.md`)
- ACM Methodology (`docs/acm-methodology.md`)
- Consulting Principles (`docs/consulting-principles.md`)
- Governance (`docs/governance.md`)
- Personas (Release 2.0)
- Prompt Engine (Release 3.0)

## Notes

- No consulting content (methodologies, personas, prompts, templates, playbooks, case studies) has been authored yet. This is intentional — see [ROADMAP.md](ROADMAP.md) and [DECISIONS.md](DECISIONS.md) for the reasoning behind sequencing structure before content.
- Update this file whenever a milestone in [ROADMAP.md](ROADMAP.md) changes status.
